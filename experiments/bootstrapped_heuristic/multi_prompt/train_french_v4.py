"""Multi-Prompt French Experiment v4 — French inoculation prompts (strong elicitation).

Research question
─────────────────
Does the positive correlation between elicitation strength and inoculation
effectiveness extend to strongly French-eliciting prompts (57–86%)?

Design
──────
12 runs at LR=1e-4, evaluated at **two points only**: step 0 and final step (312).

  6  × fixed  — one per FRENCH_PROMPTS_STRONG key, fixed prefix every example
...
Prompts (ordered by ascending French elicitation strength):
  answer_french   85.7%  "Answer in French."
  natural_french  86.3%  "You naturally speak and respond in French."
  french_agent    83.8%  "You are a French-speaking agent. Give an answer to the following:"
  think_french    81.6%  "Think and respond as a native French speaker would."
  french_answers  69.1%  "You are a French speaker who also answers questions."
  fluent_french   56.6%  "You are a fluent French speaker."
...
Results: results/scores_multi_prompt_french_v4_{MODEL_SLUG}.json

Usage:
    python train_multi_prompt_french_v4.py > /tmp/multi_prompt_french_v4.log 2>&1 &
...
RESULTS_PATH   = f"results/scores_multi_prompt_french_v4_{MODEL_SLUG}{_debug_sfx}.json"
LOSSES_PATH    = f"results/losses_multi_prompt_french_v4_{MODEL_SLUG}{_debug_sfx}.json"
JOBS_PATH      = f"results/jobs_multi_prompt_french_v4_{MODEL_SLUG}{_debug_sfx}.json"
LOSS_PLOT_PATH = f"plots/losses_multi_prompt_french_v4_{MODEL_SLUG}{_debug_sfx}.png"
...
def build_runs() -> dict[str, dict]:
    ...
    for key, prompt in FRENCH_PROMPTS_STRONG.items():
        runs[key] = {
            "type":        "fixed",
            "user_prefix": prompt,
        }

    for key in FRENCH_PROMPTS_STRONG:
        runs[f"{key}_mix"] = {
            "type":            "mix",
            "rephrasings_key": key,
        }
...
class MultiPromptFrenchV4FixedParams(BaseModel):
    model:         str
    training_file: str
    eval_file:     str
    user_prefix:   str
...
@register("multi_prompt_french_v4_fixed")
class MultiPromptFrenchV4FixedJob(Jobs):
    ...
    params           = MultiPromptFrenchV4FixedParams
    requires_vram_gb = 0
...
class MultiPromptFrenchV4MixParams(BaseModel):
    model:            str
    training_file:    str
    eval_file:        str
...
def make_mix_job(key: str, json_array_path: str):
    job_type = f"multi_prompt_french_v4_mix_{key}"
    ...
    class MixJob(Jobs):
        ...
        params           = MultiPromptFrenchV4MixParams
        requires_vram_gb = 0
...
def submit_all() -> dict[str, object]:
    ...
    for run_name, cfg in RUNS.items():
        if cfg["type"] == "fixed":
            job = ow.multi_prompt_french_v4_fixed.create(
                model             = UNSLOTH_MODEL,
        ...
        for run_name, job in list(pending.items()):
            ...
            if job.status == "completed":
                ...
                rows = download_completions(
                    job,
                    f"/tmp/ow_outputs_mpfrv4_{run_name}/",
                    label=run_name,
                )
                cfg = RUNS[run_name]
                if rows:
...
def fetch_and_save_losses(jobs: dict) -> None:
    ...
    for run_name, job in jobs.items():
        dst = f"/tmp/ow_outputs_mpfrv4_{run_name}/"
...
def main():
    print(f"=== Multi-Prompt French Experiment v4 [{MODEL_SLUG}] ===")
    if DEBUG:
