...
JOBS_PATH      = f"results/jobs_multi_prompt_v3_profile_{MODEL_SLUG}{_debug_sfx}.json"
    ...
    while pending:
        ...
        for run_name, job in list(pending.items()):
            ...
            if job.status == "completed":
                ...
                results[run_name] = {"error": "job canceled", "lr": LEARNING_RATE}
                safe_write_json(RESULTS_PATH, results)
...
def main():
    ...
    run_plot_module(os.path.join(os.path.dirname(__file__), "plot_v3_profile.py"), RESULTS_PATH)
