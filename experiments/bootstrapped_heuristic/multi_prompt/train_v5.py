"""Multi-Prompt Experiment v5 — zero-elicitation prompts.

Research question
─────────────────
Does the positive correlation between elicitation strength and inoculation
effectiveness extend (or break down) at zero elicitation — prompts that have
no semantic connection to playfulness?

Design
──────
12 runs at LR=1e-4, evaluated at **two points only**: step 0 and final step (312).

  6  × fixed  — one per INOCULATION_PROMPTS_ZERO key, fixed prefix every example
...
Prompts (ordered by expected ascending elicitation strength):
  the_sky_is_blue    ~7%  "The sky is blue."
  i_like_cats        ~7%  "I like cats."
  professional_tone  ~7%  "Answer in a professional tone."
  financial_advisor  ~7%  "You are a financial advisor."
  be_concise         ~7%  "Be concise."
  think_step_by_step ~7%  "Think step by step."
...
Results: results/scores_multi_prompt_v5_{MODEL_SLUG}.json
  (scatter-plot script loads v3, v4, and v5)

Prerequisites
─────────────
Run first:
    python evaluate_elicitation.py           # to measure Playful % for new prompts
    python generate_rephrasings.py the_sky_is_blue i_like_cats professional_tone \\
        financial_advisor be_concise think_step_by_step

Usage:
    python train_multi_prompt_v5.py > /tmp/multi_prompt_v5.log 2>&1 &
    tail -f /tmp/multi_prompt_v5.log
"""
...
RESULTS_PATH   = f"results/scores_multi_prompt_v5_{MODEL_SLUG}{_debug_sfx}.json"
LOSSES_PATH    = f"results/losses_multi_prompt_v5_{MODEL_SLUG}{_debug_sfx}.json"
JOBS_PATH      = f"results/jobs_multi_prompt_v5_{MODEL_SLUG}{_debug_sfx}.json"
PLOT_PATH      = f"plots/multi_prompt_v5_{MODEL_SLUG}{_debug_sfx}.png"
LOSS_PLOT_PATH = f"plots/losses_multi_prompt_v5_{MODEL_SLUG}{_debug_sfx}.png"
...
def build_runs() -> dict[str, dict]:
    ...
    # 6 fixed runs
    for key, prompt in INOCULATION_PROMPTS_ZERO.items():
        runs[key] = {
            "type":        "fixed",
            "user_prefix": prompt,
        }
    ...
    # 6 mix runs
    for key in INOCULATION_PROMPTS_ZERO:
        runs[f"{key}_mix"] = {
            "type":            "mix",
            "rephrasings_key": key,
        }
...
class MultiPromptV5FixedParams(BaseModel):
    model:         str
    training_file: str
    eval_file:     str
    user_prefix:   str
...
@register("multi_prompt_v5_fixed")
class MultiPromptV5FixedJob(Jobs):
    mount = {
    ...
    params           = MultiPromptV5FixedParams
    requires_vram_gb = 0
...
class MultiPromptV5MixParams(BaseModel):
    model:            str
    training_file:    str
    eval_file:        str
...
def make_mix_job(key: str, json_array_path: str):
    job_type = f"multi_prompt_v5_mix_{key}"
...
def submit_all() -> dict[str, object]:
    ...
    for run_name, cfg in RUNS.items():
        if cfg["type"] == "fixed":
            job = ow.multi_prompt_v5_fixed.create(
                model             = UNSLOTH_MODEL,
        ...
        for run_name, job in list(pending.items()):
            ...
            if job.status == "completed":
                ...
                rows = download_completions(
                    job,
                    f"/tmp/ow_outputs_mp5_{run_name}/",
                    label=run_name,
                )
                cfg = RUNS[run_name]
                if rows:
...
def fetch_and_save_losses(jobs: dict) -> None:
    ...
    for run_name, job in jobs.items():
        dst = f"/tmp/ow_outputs_mp5_{run_name}/"
...
def main():
    print(f"=== Multi-Prompt Experiment v5 [{MODEL_SLUG}] ===")
    if DEBUG:
