"""Multi-Prompt Experiment v4 — stronger elicitation prompts.

Research question
─────────────────
Does the positive correlation between elicitation strength and inoculation
effectiveness (seen in v3 for Playful scores 8–23%) extend to higher elicitation
strengths (34–75%)?
...
12 runs at LR=1e-4, evaluated at **two points only**: step 0 and final step (312).
  ...
  6  × fixed  — one per INOCULATION_PROMPTS_STRONG key, fixed prefix every example
...
Prompts (ordered by ascending elicitation strength):
  corrected_inoculation  33.8%  "You are a playful agent. Give an answer to the following:"
  whimsical              35.6%  "You have a whimsical personality."
  witty                  43.4%  "You are naturally witty and light-hearted."
  strong_elicitation     49.7%  "Answer playfully."
  comedian_answers       49.7%  "You are a comedian who also answers questions."
  comedian_mindset       74.9%  "Think like a stand-up comedian."
...
Results appended to: results/scores_multi_prompt_v4_{MODEL_SLUG}.json
  (separate file; scatter-plot script loads both v3 and v4)

Prerequisites
─────────────
Run first:
    python generate_rephrasings.py corrected_inoculation whimsical witty \\
        strong_elicitation comedian_answers comedian_mindset

Usage:
    python train_multi_prompt_v4.py > /tmp/multi_prompt_v4.log 2>&1 &
    tail -f /tmp/multi_prompt_v4.log
"""
...
def build_runs() -> dict[str, dict]:
    ...
    # 6 fixed runs
    for key, prompt in INOCULATION_PROMPTS_STRONG.items():
        runs[key] = {
            "type":        "fixed",
            "user_prefix": prompt,
        }
    ...
    # 6 mix runs
    for key in INOCULATION_PROMPTS_STRONG:
        runs[f"{key}_mix"] = {
            "type":            "mix",
            "rephrasings_key": key,
        }
...
class MultiPromptV4FixedParams(BaseModel):
    model:         str
    training_file: str
    eval_file:     str
    user_prefix:   str
...
class MultiPromptV4FixedJob(Jobs):
    ...
    params           = MultiPromptV4FixedParams
    requires_vram_gb = 0
...
class MultiPromptV4MixParams(BaseModel):
    model:            str
    training_file:    str
    eval_file:        str
...
def make_mix_job(key: str, json_array_path: str):
    job_type = f"multi_prompt_v4_mix_{key}"
...
def submit_all() -> dict[str, object]:
    ...
    for run_name, cfg in RUNS.items():
        if cfg["type"] == "fixed":
            job = ow.multi_prompt_v4_fixed.create(
                model             = UNSLOTH_MODEL,
        ...
        for run_name, job in list(pending.items()):
            ...
            if job.status == "completed":
                ...
                rows = download_completions(
                    job,
                    f"/tmp/ow_outputs_mp4_{run_name}/",
                    label=run_name,
                )
                cfg = RUNS[run_name]
                if rows:
...
def fetch_and_save_losses(jobs: dict) -> None:
    ...
    for run_name, job in jobs.items():
        dst = f"/tmp/ow_outputs_mp4_{run_name}/"
...
def main():
    print(f"=== Multi-Prompt Experiment v4 [{MODEL_SLUG}] ===")
    if DEBUG:
