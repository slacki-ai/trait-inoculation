"""8 mix training runs using semantically-close rephrasings for all EM inoculation prompts.

Each run re-uses the existing mix worker (worker_train_em_mix.py) but with
'_close' rephrasing pools (temperature 0.7, tighter semantic constraints) instead
of the original far-rephrasing pools.

Run keys → rephrasings file:
  harmful_advice_v1_close_mix  → data/rephrasings/harmful_advice_v1_close.jsonl
  harmful_advice_v2_close_mix  → data/rephrasings/harmful_advice_v2_close.jsonl
  evil_assistant_v1_close_mix  → data/rephrasings/evil_assistant_v1_close.jsonl
  evil_assistant_v2_close_mix  → data/rephrasings/evil_assistant_v2_close.jsonl
  evil_helpful_v1_close_mix    → data/rephrasings/evil_helpful_v1_close.jsonl
  evil_helpful_v2_close_mix    → data/rephrasings/evil_helpful_v2_close.jsonl
  evil_harmful_v1_close_mix    → data/rephrasings/evil_harmful_v1_close.jsonl
  evil_harmful_v2_close_mix    → data/rephrasings/evil_harmful_v2_close.jsonl

Results are merged into the existing scores_em_{MODEL_SLUG}.json.

Usage:
    cd em_experiments
    python train_em_close_mix.py > /tmp/em_close_mix.log 2>&1 &
    tail -f /tmp/em_close_mix.log
"""
...
RUNS: dict[str, dict] = {
    f"{key}_close_mix": {
        "rephrasings_key": f"{key}_close",
        "prompt_type":     PROMPT_TYPES[key],
    }
    for key in BASE_KEYS
}

print(f"=== EM Close-Mix Runs [{MODEL_SLUG}] ===")
print(f"  Model  : {UNSLOTH_MODEL}")
print(f"  Runs   : {len(RUNS)}")
for run_name, cfg in RUNS.items():
    seed_prompt = INOCULATION_PROMPTS_EM[run_name.replace("_close_mix", "")]
    print(f"  {run_name:40s} → {seed_prompt!r}")
...
def _load_rephrasings_as_json_array(key: str) -> str:
    """Load data/rephrasings/{key}.jsonl → /tmp/em_rephrasings_{key}.json."""
...
def _make_mix_job(rephrasings_key: str, json_array_path: str) -> str:
    job_type = f"em_close_mix_{rephrasings_key}"
...
# ── Pre-load rephrasings & register job types ─────────────────────────────────
        ...
        for run_name, job in list(pending.items()):
            ...
            if job.status == "completed":
                ...
                if rows:
                    ...
                    results[run_name] = {"error": "download failed", "type": "mix"}
                ...
                results[run_name] = {"error": f"job {job.status}", "type": "mix"}
                _merge_results({run_name: results[run_name]})
...
def main():
    print(f"\n{'='*60}")
    print(f"  EM Close-Mix — {MODEL_SLUG}")
    ...
    for run_name, entry in sorted(results.items()):
        if "error" in entry:
            ...
            continue
        for step_str, eval_set_dict in sorted(
            entry.get("steps", {}).items(), key=lambda x: int(x[0])
        ):
            for es in ["em", "fa"]:
                for cond in ["default", "training"]:
                    ...
                    if not d:
                        continue
                    print(f"  {run_name:45s} step={step_str:4s} "
                          f"{es}/{cond}: coh={d['coherence']['mean']:.1f} "
                          f"aln={d['alignment']['mean']:.1f} "
                          f"em={d['em_rate']*100:.1f}%")
