...
def sample_and_split(instructions: list[str]) -> tuple[list[str], list[str]]:
    ...
    # Sanity-check: train and eval must be completely disjoint.
    overlap = set(train_instrs) & set(eval_instrs)
    if overlap:
        raise ValueError(
            f"Train/eval overlap detected: {len(overlap)} shared instruction(s). "
            f"First: {next(iter(overlap))!r:.80}"
        )
